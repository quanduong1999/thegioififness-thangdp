      ...
      <div className="container home-teacher">
        {savePlaceData.length !== 0 ? (
          ...
          <h2 className="home-teacher-title">
            Danh sách phòng tập đã lưu không có
          </h2>
        )}
        <CardColumns className="home-teacher-content">
          {savePlaceData.map((place) => (
            <Card key={place.id} className="home-teacher-card place-click">
              ...
              <Card.Body>
                <Card.Title>
                  {place.place == null ? "" : place.place.name}
                </Card.Title>
                <Card.Text>
                  {place.place == null ? "" : place.place.diachi}{" "}
                </Card.Text>
              </Card.Body>
              <Button
                variant="danger"
                onClick={deletePlace(place.place == null ? "" : place.place.id)}
              >
                Xóa
              </Button>
            </Card>
